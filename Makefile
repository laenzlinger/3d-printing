...
slice: ## Slice STL: make slice STL=file.stl [MATERIAL=name] [PROCESS=name] [BRIM=1] [SUPPORT=1]
	@./scripts/slice.sh $(if $(BRIM),--brim) $(if $(SUPPORT),--support) $(if $(MATERIAL),-m "$(MATERIAL)") $(if $(PROCESS),-p "$(PROCESS)") "$(STL)"

slice-upload: ## Slice and upload: make slice-upload STL=file.stl [BRIM=1] [SUPPORT=1]
	@./scripts/slice.sh -u $(if $(BRIM),--brim) $(if $(SUPPORT),--support) $(if $(MATERIAL),-m "$(MATERIAL)") $(if $(PROCESS),-p "$(PROCESS)") "$(STL)"

slice-print: ## Slice, upload and start: make slice-print STL=file.stl [BRIM=1] [SUPPORT=1]
	@./scripts/slice.sh -s $(if $(BRIM),--brim) $(if $(SUPPORT),--support) $(if $(MATERIAL),-m "$(MATERIAL)") $(if $(PROCESS),-p "$(PROCESS)") "$(STL)"
